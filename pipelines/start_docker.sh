...
SHARED_HOST=$(hostname -i | awk '{print $1}')
PIPELINES_DIR=${PIPELINES_DIR:-./pipelines}

echo "[INFO] Starting pipelines server on $SHARED_HOST:$PORT"
...
# Start the server
uvicorn main:app --host $SHARED_HOST --port $PORT --forwarded-allow-ips '*'